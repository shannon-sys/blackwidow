...
Status RedisZSets::LogAdd(const Slice& key, const Slice& value, int32_t cf_index) {
  Status s;
  bool flag = false;
...
Status RedisZSets::LogDelete(const Slice& key, int32_t cf_index) {
  Status s;
  bool flag = false;
  for (auto cfh : handles_) {
    if ((int32_t)cfh->GetID() == cf_index) {
...
Status RedisZSets::LogCreateDB(int32_t db_index) {
